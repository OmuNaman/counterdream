...
class SessionEngine:
    def __init__(self, checkpoint, seeds, profile=None):
        ...
        for field in ("upscaler", "motion_checkpoint"):
            if (
                ...
                and not Path(self.settings[field]).is_absolute()
            ):
                self.settings[field] = str(Path(profile).parent / self.settings[field])
        if self.settings.get("motion_only") and not self.settings.get(
            "motion_checkpoint"
        ):
            raise ValueError("Motion-only profile requires motion weights")
        if self.settings.get("temporal_detail", 0):
            raise ValueError(
                "Temporal-detail experiments are not enabled in the live viewer"
            )
        ...
        if self.settings.get("motion_checkpoint"):
            from .motion_v2 import load_motion

            self.motion = load_motion(self.settings["motion_checkpoint"])
    ...
    def render(self, image):
        ...
        if self.upscaler is not None:
            with torch.autocast("cuda", dtype=torch.float16):
                pixels = self.upscaler(image.float().add(1).mul(0.5))
            frame = (
                pixels[0]
                .float()
                .mul(255)
            ...
            Image.fromarray(frame).save(
                stream, format="JPEG", quality=88, subsampling=2
            )
    ...
    def frame(self, session_id, command, sequence):
        if (
            ...
            or not all(x in "0123456789abcdef" for x in session_id)
        ):
            raise ValueError("Invalid session identifier")
        control = Control.model_validate(command)
        now = time.monotonic()
        self.sessions = {
            k: v for k, v in self.sessions.items() if now - v["seen"] < 900
        }
        ...
        if control.type == "reset":
            ...
            if prior is None and len(self.sessions) >= 4:
                return {
                    "error": "GPU session limit reached. Try again after an inactive session expires."
                }
            state = dict(
                context=self.seeds[control.spawn : control.spawn + 1].clone(),
            ...
            result = dict(png=self.render(state["context"][:, -1]), frame=0, gpu_ms=0.0)
            state["last_result"] = result
            self.sessions[session_id] = state
            return result
        if prior is None:
            return {
                "error": "The idle GPU session expired. Reset the world to continue."
            }
        if (
            now - prior["spawned"] > 900
            or now - self.created > 2700
            or self.generated >= 20000
        ):
            return {
                "error": "Cloud session allocation finished. Restart the viewer for another allocation."
            }
        if sequence != prior["sequence"] + 1:
            return {"error": "Missing control request. Reset the world."}
        controls = torch.as_tensor(
            encode(control.keys, *control.mouse_delta(), control.fire, control.scope),
            device="cuda",
        )[None, None]
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            if self.motion is not None:
                ...
                initial = guided_motion(
                    ...
                    prior["context"][:, -2:],
                    history[:, -2:],
                    scale=self.settings.get("motion_scale", 1.0),
                    center=self.settings.get("motion_center", False),
                )
            if self.settings.get("motion_only"):
                ...
                predicted = sample(
                    ...
                    prior["context"],
                    history,
                    replace(self.options, steps=control.steps),
        ...
        if control.fire and self.options and self.settings.get("weapon_refinement", 0):
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                predicted = refine_firing(
                    ...
                    self.options,
                    seed=1000 + prior["frame"],
                    strength=self.settings["weapon_refinement"],
                )
