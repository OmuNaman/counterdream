...
The [latest controls pass](docs/PLAYABILITY_PASS.md) fixes dropped taps, sends input
changes immediately, adds mouse capture/right-drag look and visible controls, and
bounds the browser's frame queue. A new motion model improved short-horizon error
but failed longer play tests; it is documented and excluded from the live viewer.
...
arrow keys, right-mouse drag, or captured mouse; fire with F or left click; jump with Space; reload with R;
...
The viewer includes adjustable look speed, fullscreen, filtered/pixel display, and
